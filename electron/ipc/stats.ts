...
export function registerStatsIPC(): void {
  ...
  ipcMain.handle('stats:saveExportFile', async (_event, data: string, defaultName: string): Promise<{ success: boolean; data?: string; error?: string }> => {
    try {
  ...
  ipcMain.handle('stats:selectImportFile', async (): Promise<{ success: boolean; data?: string; error?: string }> => {
    try {
  ...
  ipcMain.handle('stats:import', async (_event, req: ImportDataRequest): Promise<{ success: boolean; data?: { importedCount: number; failedCount: number; details: string }; error?: string }> => {
    try {
      const db = getDatabase();
      let importedCount = 0;
      let failedCount = 0;
      ...
      const splitCSVRecords = (csv: string): string[] => {
      ...
      const parseCSV = (csv: string): {
        ...
        for (const record of records) {
          if (record.startsWith('=== ')) {
            if (record.includes('书籍数据')) currentSection = 'books';
            else if (record.includes('借阅记录')) currentSection = 'borrowRecords';
            else if (record.includes('愿望单')) currentSection = 'wishlistItems';
            else if (record.includes('书架位置')) currentSection = 'shelfLocations';
      ...
      const transaction = db.transaction(() => {
        ...
        if (parsedData.books && Array.isArray(parsedData.books)) {
          ...
          if (bookOk > 0) details.push(`书籍 ${bookOk} 条`);
          if (bookFail > 0) details.push(`书籍 ${bookFail} 条失败`);
        }
        ...
        if (parsedData.borrowRecords && Array.isArray(parsedData.borrowRecords)) {
          ...
          if (borrowOk > 0) details.push(`借阅 ${borrowOk} 条`);
          if (borrowFail > 0) details.push(`借阅 ${borrowFail} 条失败`);
        }
        ...
        if (parsedData.wishlistItems && Array.isArray(parsedData.wishlistItems)) {
          ...
          if (wishOk > 0) details.push(`愿望单 ${wishOk} 条`);
          if (wishFail > 0) details.push(`愿望单 ${wishFail} 条失败`);
        }
        ...
        if (parsedData.shelfLocations && Array.isArray(parsedData.shelfLocations)) {
          ...
          if (shelfOk > 0) details.push(`书架 ${shelfOk} 条`);
          if (shelfFail > 0) details.push(`书架 ${shelfFail} 条失败`);
        }
      });
  ...
  ipcMain.handle('file:saveCover', async (_event, bookId: string, filePath: string): Promise<{ success: boolean; data?: string; error?: string }> => {
    try {
  ...
  ipcMain.handle('file:selectImage', async (): Promise<{ success: boolean; data?: string; error?: string }> => {
    try {
      const result = await dialog.showOpenDialog({
        title: '选择封面图片',
        properties: ['openFile'],
