...
function findDuplicateBooks(db) {
...
export function registerStatsIPC() {
    ipcMain.handle('stats:get', async () => {
        try {
            ...
            const totalResult = db.prepare('SELECT COUNT(*) as count FROM books').get();
            const readResult = db.prepare("SELECT COUNT(*) as count FROM books WHERE status = 'read'").get();
            const readingResult = db.prepare("SELECT COUNT(*) as count FROM books WHERE status = 'reading'").get();
            const unreadResult = db.prepare("SELECT COUNT(*) as count FROM books WHERE status = 'unread'").get();
            const valueResult = db.prepare('SELECT COALESCE(SUM(purchase_price), 0) as total FROM books').get();
            const authorResult = db.prepare('SELECT COUNT(DISTINCT author) as count FROM books').get();
            const tagCountResult = db.prepare('SELECT COUNT(*) as count FROM tags').get();
            const borrowResult = db.prepare("SELECT COUNT(*) as count FROM borrow_records WHERE actual_return_date IS NULL").get();
            const wishlistResult = db.prepare('SELECT COUNT(*) as count FROM wishlist_items WHERE purchased = 0').get();
            const shelfResult = db.prepare('SELECT COUNT(*) as count FROM shelf_locations').get();
    ...
    ipcMain.handle('stats:export', async (_event, req) => {
        try {
            const db = getDatabase();
            const result = {};
        ...
        `).all();
            ...
            if (includeBorrow) {
                const borrowRows = db.prepare('SELECT * FROM borrow_records').all();
                result.borrowRecords = borrowRows.map(rowToBorrowRecord);
            }
            if (includeShelf) {
                const shelfRows = db.prepare('SELECT * FROM shelf_locations').all();
    ...
    ipcMain.handle('stats:saveExportFile', async (_event, data, defaultName) => {
        try {
    ...
    ipcMain.handle('stats:selectImportFile', async () => {
        try {
    ...
    ipcMain.handle('stats:import', async (_event, req) => {
        try {
            const db = getDatabase();
            let importedCount = 0;
    ...
    ipcMain.handle('file:saveCover', async (_event, bookId, filePath) => {
        try {
